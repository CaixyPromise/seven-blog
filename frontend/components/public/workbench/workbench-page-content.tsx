...
import type { WorkbenchActivity, WorkbenchItem } from "@/lib/content/workbench"
import type { Messages } from "@/lib/i18n"

interface WorkbenchPageContentProps {
  items: WorkbenchItem[]
  activity: WorkbenchActivity[]
  copy: Messages["workbench"]
}

export function WorkbenchPageContent({ items, activity, copy }: WorkbenchPageContentProps) {
  const [hoveredItem, setHoveredItem] = useState<number | null>(null)
  const [isVisible, setIsVisible] = useState(false)
  const averageProgress = items.length > 0 ? Math.round(items.reduce((a, b) => a + b.progress, 0) / items.length) : 0
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2">
            ...
            >
              ...
              <div className="flex items-center gap-3 border-b border-border/50 bg-secondary/40 px-4 sm:px-5 py-3.5 sm:py-4">
                ...
                <span className="ml-4 font-mono text-xs text-muted-foreground truncate">~/CaixyPromise/active</span>
                <div className="ml-auto flex items-center gap-2 text-muted-foreground">
                  <span className="h-1.5 w-1.5 rounded-full bg-primary animate-pulse" />
                  <span className="font-mono text-xs">{copy.live}</span>
                </div>
              </div>
                  ...
                  >
                    <div className="flex-1 space-y-2 min-w-0">
                      ...
                      <div className="pl-6 flex items-center gap-4 text-xs text-muted-foreground">
                        ...
                        <span>{item.commits} {copy.commits}</span>
                      </div>
                    </div>
              ...
              <div className="border-t border-border/50 bg-secondary/30 px-4 sm:px-5 py-4">
                <div className="flex items-center gap-2 font-mono text-xs text-muted-foreground">
                  ...
                  <span className="typing-cursor truncate">{copy.command}</span>
                  <span className="ml-auto text-primary/50 hidden sm:block">{copy.hint}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            >
              <h3 className="font-mono text-xs uppercase tracking-wider text-primary mb-4">{copy.stats}</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-3 rounded-lg bg-secondary/30">
                  <p className="text-2xl font-bold text-foreground">{items.length}</p>
                  <p className="font-mono text-xs text-muted-foreground">{copy.active}</p>
                </div>
                <div className="text-center p-3 rounded-lg bg-secondary/30">
                  <p className="text-2xl font-bold text-primary">{averageProgress}%</p>
                  <p className="font-mono text-xs text-muted-foreground">{copy.avgProgress}</p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div className="space-y-3">
                {activity.map((activityItem) => (
                  <div key={activityItem.id} className="flex items-start gap-3 text-xs">
                    <span
                      className={cn(
                        "shrink-0 w-1.5 h-1.5 rounded-full mt-1.5",
                        activityItem.type === "commit" ? "bg-primary" : "bg-yellow-500",
                      )}
                    />
                    <div className="min-w-0 flex-1">
                      <p className="text-foreground truncate">{activityItem.message}</p>
