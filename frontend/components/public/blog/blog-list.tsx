...
import { withLocaleHref, type Locale, type Messages } from "@/lib/i18n"
...
export function BlogList({ posts, copy, locale }: BlogListProps) {
  if (posts.length === 0) {
    return null
  }
    ...
    <div className="space-y-6">
      {posts.map((post, index) => (
        <article
          key={post.id}
          className={cn(
            "group relative cursor-pointer overflow-hidden rounded-xl border border-border bg-card/40 glass p-6 sm:p-7 transition-all duration-400 hover:border-primary/40 hover:bg-card/60 active:scale-[0.995] hover-lift animate-fade-in-up",
            post.featured && "ring-1 ring-primary/20",
          )}
          style={{ animationDelay: `${index * 80 + 100}ms` }}
        >
          <Link href={withLocaleHref(`/blog/${post.slug}`, locale)} className="absolute inset-0 z-10">
