...
import type { HomePageContent } from "@/lib/content/site"

const fallbackRoles = ["crafting code"]
...
export function HeroSection({ content }: HeroSectionProps) {
  const roles = content.rotatingPhrases.length > 0 ? content.rotatingPhrases : fallbackRoles
        ...
        <div className="grid gap-12 lg:grid-cols-2 lg:gap-20 lg:items-center lg:min-h-[70vh]">
          ...
          <div className="space-y-8 sm:space-y-10">
            ...
            <p className="max-w-lg text-base sm:text-lg leading-relaxed text-muted-foreground animate-fade-in-up stagger-2">
              {content.description} {content.currentOrgPrefix ?? "Currently building at"}{" "}
              <span className="text-foreground font-medium">{content.currentOrgLabel}</span>
              {content.currentOrgSuffix ?? "."}
            </p>
          ...
          <div className="relative animate-scale-in stagger-4">
            <div className="relative rounded-xl border border-border bg-card/60 glass p-5 sm:p-8 hover-lift">
              ...
              <div className="mt-12 rounded-lg border border-primary/35 bg-background/45 p-5 font-mono text-[12px] leading-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] sm:p-7 sm:text-sm">
                <div className="border-b border-border/50 pb-5">
                  <div className="text-sm font-semibold uppercase text-foreground sm:text-base">
                    {content.terminalProfile.title}
                  </div>
                  <div className="mt-1 text-muted-foreground">{content.terminalProfile.subtitle}</div>
                </div>

                <div className="mt-5 space-y-2.5">
                  {content.terminalProfile.rows.map((row) => (
                    <div key={row.key} className="grid gap-x-3 gap-y-1 sm:grid-cols-[6.5rem_minmax(0,1fr)]">
                      <span className="whitespace-nowrap text-primary/80">
                        <span className="text-muted-foreground">&gt;</span> {row.key}
                        <span className="text-muted-foreground">:</span>
                      </span>
                      <span className="min-w-0 break-words text-foreground/85">{row.value}</span>
                    </div>
                  ))}
                </div>
              </div>
